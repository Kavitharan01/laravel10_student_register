                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form method="post" action="{{ isset($post) ? route('posts.update', $post->id) : route('posts.store') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
                        ...
                        <div>
                            ...
                            <x-text-input id="id" name="id" type="text" class="mt-1 block w-full" :value="$post->title ?? old('id')" required autofocus />
                            <x-input-error class="mt-2" :messages="$errors->get('id')" />
                        </div>

                        <div>
                            <x-input-label for="name" value="Name" />
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="$post->title ?? old('name')" required autofocus />
                        ...
                        <div>
                            ...
                            <x-text-input id="age" name="age" type="text" class="mt-1 block w-full" :value="$post->title ?? old('age')" required autofocus />
                            <x-input-error class="mt-2" :messages="$errors->get('age')" />
                        </div>

                        <div>
                            <x-input-label for="status" value="Status" />
                            <x-text-input id="status" name="status" type="text" class="mt-1 block w-full" :value="$post->title ?? old('status')" required autofocus />
