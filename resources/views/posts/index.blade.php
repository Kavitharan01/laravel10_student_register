...
<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            <a href="{{ route('posts.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded-md">ADD Student</a>
        </div>
    </x-slot>
                    ...
                    <table class="border-collapse table-auto w-full text-sm">
                        <thead>
                            <tr>
                                ...
                                <th class="border-b font-medium p-4 pl-8 pt-0 pb-3 text-left">Age</th>
                                <th class="border-b font-medium p-4 pl-8 pt-0 pb-3 text-left">Status</th>
                                <th class="border-b font-medium p-4 pl-8 pt-0 pb-3 text-left">Action</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white">
                            ...
                            @foreach ($posts as $post)
                                <tr>
                                    ...
                                    <td class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400">{{ $post->age }}</td>
                                    <td class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8">
                                        @if ($post->status==0)
                                        <span class="badge bg-success text-slate-500 dark:text-slate-400">Deactivate</span>
                                        @else
                                        <span class="badge bg-warning text-slate-500 dark:text-slate-400">Activate</span>
                                        @endif
                                    </td>
